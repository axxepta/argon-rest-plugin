...
public class ArgonURLConnection extends FilterURLConnection implements CacheableUrlConnection {
     ...
     */
    public static final String ARGON_PROTOCOL = "argon";
     ...
     */
    protected ArgonURLConnection(String contextId, URLConnection delegate) {
        super(delegate);
            ...
            return new FilterOutputStream(super.getOutputStream()) {
                ...
                public void close() throws IOException {
                    try {
                        ...
                        String actualLocation = ArgonURLConnection.this.getHeaderField("Location");
                        if (actualLocation != null) {
                            ArgonURLConnection.this.urlOverride = new URL(actualLocation);
                        }
                    } catch (IOException e) {
                        handleException(e);
                    }
    ...
    public List<FolderEntryDescriptor> listFolder() throws IOException {
        ...
        try {
            ...
            logger.debug("Failed to read folder listing from BaseX server :" + e.getMessage());
        ...
        if (logger.isDebugEnabled()) {
            ...
            logger.debug("Received folder listing from BaseX server :" + jsonFilesString);
        }

        ObjectMapper mapper = new ObjectMapper();
        JsonNode[] array = mapper.readValue(jsonBytes, mapper.getTypeFactory().constructArrayType(JsonNode.class));

        List<FolderEntryDescriptor> files = new ArrayList<>();
        for (JsonNode file : array) {
            ...
            String filePath = getDocumentURL() + encodedFileName + (isFolder ? "/" : "");
     ...
     */
    static byte[] readConnectionBytes(URLConnection connection) throws IOException {
        byte[] jsonBytes;
    ...
    public String getHeaderField(String name) {
        if (name.equals("oxygen_read_only")) {
            if (ArgonLockHandler.resourceLocked(contextId, url))
                return "true";
                else return null;
        }
