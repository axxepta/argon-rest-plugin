...
public class ArgonURLStreamHandler extends URLStreamHandlerWithContext {
    ...
    static URL getLockUrl(URL url, String method, boolean argonStyle) {
        String encodedDocumentURL = argonStyle ?
                URLUtil.encodeURIComponent(url.toExternalForm().substring(ArgonURLConnection.ARGON_PROTOCOL.length() + 13)) :
                url.getQuery().substring(16);
        String serverUrl = getServerUrl();
        try {
            if (serverUrl != null && !serverUrl.isEmpty()) {
                String lockUrl = serverUrl + method + "?path=" + encodedDocumentURL;
                return new URL(lockUrl);
            }
        } catch (MalformedURLException me) { /*/ handle in return statement*/ }
        return url;
    }
}
